...
def verify_snapshot_signature(
    ...
    if be == "hmac":
        ...
        # Falls back to a structural bundle check when allowed.
        if not _HAVE_SIGSTORE:
        ...
        try:
            ...
            bundle = _Bundle.from_json(dsse_bundle_json)
            # Build verifier
            from sigstore.verify.verifier import Verifier as _Verifier  # type: ignore
            if env == "staging":
                verifier = _Verifier.staging()
            else:
                verifier = _Verifier.production()
            # Build policy
            from sigstore.verify import policy as _policy  # type: ignore
            if verify_identity:
                pol = _policy.Identity(identity=str(verify_identity), issuer=str(verify_issuer) if verify_issuer else None)
            elif allow_unsafe:
                pol = _policy.UnsafeNoOp()
            else:
                # No explicit identity and unsafe not allowed => don't proceed
                raise RuntimeError("missing_policy")
            # Verify DSSE signature and extract payload
            ptype, payload = verifier.verify_dsse(bundle, pol)
            # Ensure payload digest matches the claimed snapshot hash
            import hashlib as _hl
            computed = _hl.sha256(payload).hexdigest()
            if computed == str(snapshot_hash):
                return {"valid": True, "backend": "sigstore", "reason": f"verified:{env}", "payload_type": ptype}
            # If mismatch, consider a structural check of bundle as fallback
            reason = "payload_hash_mismatch"
        except Exception as e:
            # Keep the exception around and attempt a structural fallback if enabled
            reason = f"verify_error:{e}"
        # Structural fallback: try to parse and compare a claimed payload hash
        if allow_offline_structural:
            try:
                import json as _json  # local import
                bundle_obj = _json.loads(dsse_bundle_json)
                claimed = None
                if isinstance(bundle_obj, dict):
                    claimed = bundle_obj.get("payloadSHA256") or bundle_obj.get("snapshot_hash")
                    if not claimed:
                        dsse = bundle_obj.get("dsse")
                        if isinstance(dsse, dict):
                            claimed = dsse.get("payloadSHA256")
