// Lightweight SVG export for 2D overlays (labels/markers/lasso) aligned to the hero area
// This does not attempt to vectorize the 3D scene; it captures current selections and timeline markers.
...
export type SvgMarker = { t: number; label?: string };

export function exportTrendsSVG(opts: {
  width: number;
  height: number;
  points: SvgPoint[];
  ...
  // Limit labels to keep SVG readable
  ...
  const markerEls = markers.map((m, i) => {
    // Render timeline markers along the bottom as triangles with labels
...
<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">
  ...
  a.href = url;
  a.download = `trends_${Date.now()}.svg`;
  a.click();
