// extracted original heavy client page
"use client";
...
export default function MarketMapClient() {
  ...
  const Graph = ({ elements, stylesheet }: { elements: any[]; stylesheet: any[] }) => {
    ...
    useEffect(() => {
      ...
      return () => { try { cy.destroy(); } catch {}; cyRef.current = null; };
    }, []);
    useEffect(() => {
      const cy = cyRef.current;
      if (!cy) return;
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <button onClick={() => {
            ...
            setSelectedView("");
            setPage(1);
            try { const url = window.location.pathname; window.history.replaceState(null, "", url); } catch {}
            load(true, false);
